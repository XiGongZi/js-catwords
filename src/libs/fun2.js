...
let fun = {
  ...
  judgeExist() {
    ...
    let nowStr = this.keyword;
    console.log(`判断是否存在【${nowStr}】`);
    console.log(`当前关键词： 【${this.keyword}】`);
    if (!len) return;
    let obj = this.keys[this.keyword];
    if (!obj) {
      console.log(`不存在 【${nowStr}】 ,停止当前单词检测`);
      this.keyword = nowStr.substr(0, len - 1);
      this.status = 0;
    } else {
  ...
  setWords() {
    ...
    this.keyword = this.keyword.charAt(this.keyword.length - 1);
    console.log("");
    this.status = 1;
  },
  subStr(data) {
